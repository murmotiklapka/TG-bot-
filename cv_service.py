...
def load_keras_models(model_path = "./cv_models/keras_model.h5", labels_path = "./cv_models/labels.txt"):
...
def classificate_image(image_path, model, class_names):
    ...
    print('--- Keras raw prediction ---')
    for name, score in zip(class_names, prediction[0]):
        print(f'  {name.strip():<20} {float(score):.6f}')
    index = np.argmax(prediction)
    class_name = class_names[index]
    confidence_score = prediction[0][index]

    return class_name.split(' ', 1)[1].strip(), round(float(confidence_score) * 100, 2)
...
VEHICLE_CLASSES = {'airplane', 'boat', 'car', 'truck', 'bus'}

def handle_image(image_path):
    detections = detect_objects(image_path)
    model, class_names = load_keras_models()
    result = []
    for i, detection in enumerate(detections):
        try:
            if detection.get('name') in VEHICLE_CLASSES:
